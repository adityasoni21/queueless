...
export default async function CheckInPage({ params }: Props) {
  ...
  const { data: token } = await supabase
    .from("tokens")
    .select(
      `id,token_number,status,arrived_at,services(name),counters(name,location)`
    )
    .eq("id", tokenId)
    .single();
  ...
  if (!token) {
    return (
      <Result
        ...
        message="This QR code is invalid or the token no longer exists."
      />
    );
  }
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-slate-50 px-6">
      <div className="w-full max-w-md rounded-3xl bg-white p-8 text-center shadow-sm">
        <p className="text-sm font-semibold text-blue-600">Queueless</p>
        ...
        {token.status === "CALLED" ? (
          ...
          <div className="mt-6 rounded-2xl bg-yellow-50 p-4 text-sm text-yellow-800">
            This token is currently {token.status}.
          </div>
        )}
      </div>
    </main>
